...
This project set out to build a version of the well-known word game, hangman. The game aims to provide a fun challenge aimed at an adult, the audience being determined by the variety and length of words. Users should find enjoyment in completing the challenge of the game and it also provides the opportunities for users to build their vocabulary or English language knowledge. 

- __Design__
Planning and design of the game was aided by using Lucidchart. The below visual lays out the user journey and logic flow which informed the structure and code utilised in the build of the game. 
...
- dealing with errors.
- Evaluation of input and decisions on game progression and feedback to players. 
...
- __Welcome & Instruction__
  ...
  - Players are welcomed with a brief description of how to play the game. Hangman is a well-known game, so explanation has been kept minimal. Hangman can be played with words of varying length, however, this version of the game serves up words of six letters in length. A list of 492 letters being utilised has been sourced from freedictionary.com. 
...
HanHangman 
  - In order to personalise the experience of the game, players are asked for and then greeted by their first name. 
...
A valid character ie. a letter from a to z.
A letter which hasn't been previously guessed. 
...
- __Results__
  ...
  - The game is coded to monitor how many wrong answers a player has accumulated and will trigger feedback to the player as to whether they have won or lost the game. 
 ...
 <img width="783" alt="play_screen_winner" src="https://user-images.githubusercontent.com/92268504/159038360-5b83c57d-69ef-4310-80d6-1aa786d234f6.png">
  ...
  - In both instances, the player is served a personal feedback message and graphic informing them of their results. Currently it was decided to not reveal the answer to an incomplete game to a player when the game is over, this will increase the longevity of the current game content. 

### Future Expansion & Build

The game has been created as intended and includes all core functionality. Notes of possible future expansion:
...
- Adding lists of words grouped by difficulty, incorporating the option for users to select whcih group/level of difficulty they are served words from. 
- Consider options to restart the game upon completion or to quit and restart midway through a game. 
- Options around storing highest score against username. 

## Testing 

User testing was completed by myself as well as a pool of friends who played the game and gave feedback on any errors, quirks, or issues that they experienced during the course of play. Key issues adressed:

1. Being able to press any key to begin not functioning correctly -> Replaced with function of press Enter to start game.
...
3. Letter entered after in the turn after an invalid character is entered not being recorded as a guess -> Update order of code to rectify this issue.
4. Not all input functions displaying the users input in uppercase, only the initial prompt to input a letter and not those following invalid or already guessed -> .upper action added to remaining input instances. 
...
No unfixed bugs remain in code at time of deployment. 

## Deployment

This game was built using Github, utilising Python and deployed via Heroku with a mock terminal accessible via a webpage. The steps for deployment are as follows: 
  1. Add a new line at the end of any code requesting input. This is to accomodate a quirk in the software used to create the mock terminal.  
  ...
  6. Within the Settings section, update a new key with name "PORT" and value of "8000".
  7. Within settings, add Python and nodejs build packs in this order
  8. Within the Deploy section, select Github and confirm connection between your Github and Heroku accounts.
  ...
  11. App will be built and link to view app will be presented below the build section in button labelled "View". 
  
The live link can be found here - https://six-hangman.herokuapp.com/

### Credits: Content & Media 

- The dictionary of possible mystery words used within the game was sourced from the free dictionary https://www.thefreedictionary.com/6-letter-words.htm 
- Instructional text is original and was created specifically for this project.
...
Reference also made to community content shared by the Code Insitite community via Slack. 
